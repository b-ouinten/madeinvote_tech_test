...
describe Menu do
  ...
  describe "ask_integer_value" do
    it "should update entries[:integer] attribute by user input" do
  ...
  describe "ask_for_choose_approach" do
    it "should update entries[:choice] attribute by user input" do
      allow($stdin).to receive(:gets).and_return '1'
      @menu.ask_for_choose_approach
      expect(@menu.entries[:choice]).to eq '1'
    end

    it "should ask again when user input is invalid and update entries[:choice] attribute with a next valid user input" do
